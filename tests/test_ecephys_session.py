...
class TestEcephysSessionResults(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open(RESOURCES_DIR / "example_fetch_ecephys_session_result.json", "r") as f:
            response = [
        ...
        with open(RESOURCES_DIR / "example_fetch_ecephys_streams_result.json", "r") as f:
            response = [
                Record(json_entity=r, slims_api=example_client.db.slims_api)
                for r in json.load(f)
            ]
        ...
        with open(RESOURCES_DIR / "example_fetch_ecephys_stimulus_epochs_result.json", "r") as f:
            response = [
                Record(json_entity=r, slims_api=example_client.db.slims_api)
                for r in json.load(f)
            ]
    ...
    def test_fetch_ecephys_mouse_session_success(self, mock_fetch: MagicMock):
        ...
        self.assertEqual(
            self.example_fetch_ecephys_session_result[0].json_entity, session.json_entity
        )
    ...
    def test_fetch_ecephys_streams_success(self, mock_fetch: MagicMock):
        ...
        self.assertEqual(
            self.example_fetch_ecephys_streams_result[0].json_entity, streams.json_entity
        )
    ...
    def test_fetch_ecephys_stim_epochs_success(self, mock_fetch: MagicMock):
        ...
        self.assertEqual(
            self.example_fetch_ecephys_stimulus_epochs_result[0].json_entity, stim_epochs.json_entity
        )
