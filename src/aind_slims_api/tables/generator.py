...
if there is a better way to get the table schemas from SLIMS. The output file
may need a little reformatting.
"""
    ...
    match data_type:
        ...
        case SlimsColumnDataType.INTEGER:
            if re.match(PK_PATTERN, name):
                return (
                    f"    {name}: Optional[int] = "
                    f'Field(default=None, title="{title}", primary_key=True)'
                )
            else:
                return (
                    f'    {name}: Optional[int] = Field(default=None, title="{title}")'
        ...
        case SlimsColumnDataType.MULTIPLE_ENUM:
            return (
                f"    {name}: Optional[List[str]] = Field(default=None,"
                f' title="{title}")'
            )
...
if __name__ == "__main__":
    ...
    main_records = client.fetch(
        table=args.table, criteria=None, start=args.start, end=args.end
    )
    generate_module(records=main_records, write_path=args.file_path)
